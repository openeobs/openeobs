...
class test_api(common.SingleTransactionCase):
    ...
    def test_api_location_map(self):
        ...
        # get patients
        spell_activities = api.get_activities(cr, uid, data_models=['t4.clinical.spell'], states=['started'], pos_ids=[env.pos_id.id])
        ...
        for i in range(len(available_ids)):
            ...
            move = api.create_complete(cr, uid, 't4.clinical.patient.move', {},
    ...
    def test_api_activity_map(self):
        ...
        activity_model = self.registry('t4.activity')
        location_model = self.registry('t4.clinical.location')
        patient_model = self.registry('t4.clinical.patient')
        api = self.registry('t4.clinical.api')
        config = {
            'bed_qty': 3,
            'patient_qty': 2
        ...
        args = {
                'pos_ids': [env.pos_id.id],
                'data_models': ['t4.clinical.patient.register'],
