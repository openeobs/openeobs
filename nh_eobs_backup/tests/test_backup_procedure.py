...
class TestNHClinicalBackupProcedure(TransactionCase):
    ...
    def setUp(self):
        ...
        self.wt_id = self.location_pool.search(cr, uid, [('code', '=', 'T')])[0]
        self.location_pool.write(cr, uid, self.wu_id, {'backup_observations': True})
        self.pos_id = self.location_pool.read(cr, uid, self.wu_id, ['pos_id'])['pos_id'][0]
        self.bed_ids = self.location_pool.read(cr, uid, self.wu_id, ['child_ids'])['child_ids']
    ...
    def test_02_test_flag_set_on_location(self):
        # get the location registry and check that it has report_printed key and it's False
        flag_present = 'backup_observations' in self.location_pool
        flag_value = self.location_pool._defaults['backup_observations']
        self.assertEqual(flag_present, True, 'Flag not set on location class properly')
        self.assertEqual(flag_value, False, 'Flag value not set correctly')
    ...
    def test_05_test_flag_changed_by_report_printing_method_no_spell_defined(self):
        # complete an observation and check flag is now False
        cr, uid = self.cr, self.uid
    ...
    def test_07_test_no_spell_defined_domain_returns_empty_when_no_non_printed_spells(self):
        cr, uid = self.cr, self.uid
        loc_ids = self.location_pool.search(cr, uid, [['usage', '=', 'ward'], ['backup_observations', '=', True]])
        dirty_spell_ids = self.spell_pool.search(cr, uid, [['report_printed', '=', False],
    ...
    def test_08_test_no_spell_defined_domain_returns_empty_when_no_non_printed_spells(self):
        cr, uid = self.cr, self.uid
        loc_ids = self.location_pool.search(cr, uid, [['usage', '=', 'ward'], ['backup_observations', '=', True]])
        dirty_spell_ids = self.spell_pool.search(cr, uid, [['report_printed', '=', False],
                                                    ['state', 'not in', ['completed', 'cancelled']],
    ...
    def test_12_test_general_settings_view_updated_with_options(self):
        ...
        parent_view_id = view_pool.search(cr, uid, [['model', '=', 'base.config.settings'], ['mode', '=', 'primary']])[0]
        child_view_ids = view_pool.read(cr, uid, parent_view_id, ['inherit_children_ids'])['inherit_children_ids']
        our_view_id = view_pool.search(cr, uid, [['name', '=', 'base.config.settings.nhclinical']])[0]
        self.assertTrue(our_view_id in child_view_ids, 'View not in list of inherited views for general settings screen')

    def test_13_test_general_settings_loads_backup_enabled_wards_correctly(self):
        ...
        self.assertEqual(get_vals['locations_to_print'][0], self.wu_id, 'Ward U id not in location to print from settings')

    def test_14_test_general_settings_set_location_removes_all_wards_not_defined(self):
        cr, uid = self.cr, self.uid
        settings_pool = self.registry('base.config.settings')
        record = settings_pool.create(cr, uid, {'locations_to_print': []})
        settings_pool.set_locations(cr, uid, [record])
        get_vals = settings_pool.get_default_all(cr, uid, [])
        self.assertEqual(get_vals['locations_to_print'], [], 'Ward U not removed from backed up wards')

    # def test_15_test_general_settings_set_location_adds_defined_ward(self):
    #     cr, uid = self.cr, self.uid
    #     settings_pool = self.registry('base.config.settings')
    #     set_values = [self.location_pool.read(cr, uid, self.wt_id, [])]
    #     record = settings_pool.create(cr, uid, {'locations_to_print': set_values})
    #     settings_pool.write(cr, uid, record, {'locations_to_print': set_values})
    #
    #     def settings_pool_mock_browse(*args, **kwargs):
    #         return settings_pool.read(cr, uid, record, [])
    #     settings_pool._patch_method('browse', settings_pool_mock_browse)
    #
    #     settings_pool.set_locations(cr, uid, [record])
    #     get_vals = settings_pool.get_default_all(cr, uid, [])
    #     self.assertEqual(get_vals['locations_to_print'], set_values, 'Ward U not removed and Ward A not added to backed up wards')
    #     settings_pool._revert_method('browse')