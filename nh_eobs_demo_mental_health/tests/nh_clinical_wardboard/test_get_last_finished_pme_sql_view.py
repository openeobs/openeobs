...
class TestGetLastFinishedPMESQLView(TransactionCase):
    """
    Test that the last_finished_obs_stop SQL view is working correctly
    """

    def get_obs_stop_for_state(self, state):
        """
        Get obs stop activities in a state
    ...
    def get_last_finished_obs_stop(self, spell):
        """
        Use the last_finished_obs_stop SQL view to get the last PME in a state
        ...
        """
        self.cr.execute("""
            SELECT * from last_finished_obs_stop WHERE spell_id = {spell_id}
    ...
    def setUp(self):
        ...
        self.completed_pmes = self.get_obs_stop_for_state('completed')
        # Need to get a spell with a cancelled pme
        self.cancelled_pmes = self.get_obs_stop_for_state('cancelled')
        # Need to get a spell with a started pme
        self.started_pmes = self.get_obs_stop_for_state('started')
    ...
    def test_gets_latest_finished_pme_for_spell(self):
        """
        Test that the SQL view is getting the latest finished obs stop for the
        spell when there are two obs stop completed
        """
