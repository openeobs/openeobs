...
class TestBristolStools(common.SingleTransactionCase):
    ...
    def test_bristol_stools(self):
        ...
        for i in range(25):
            stools_activity_id = self.stools_pool.create_activity(
                cr, uid,
            ...
            stools_activity = self.activity_pool.browse(
                cr, uid, stools_activity_id)
            self.assertTrue(
                stools_activity.state == 'completed',
                msg="Bristol Stools not completed")
            self.assertTrue(
                stools_activity.date_terminated,
                msg="Bristol Stools date terminated not registered")
            for key in data.keys():
                self.assertTrue(
                    eval('stools_activity.data_ref.'+key) == data[key],
                    msg="Bristol Stools Completed: %s not submitted "
                        "correctly" % key)
