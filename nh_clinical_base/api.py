...
class nh_clinical_api(orm.AbstractModel):
    ...
    def browse(self, cr, uid, model, ids, context=None):
        if not isinstance(model, basestring):
            return super(nh_clinical_api, self).browse(cr, uid, ids, context)
        model_pool = self.pool[model]
    ...
    def get_patients(self, cr, uid, patient_ids=[], pos_ids=[], location_ids=[], parent_location_ids=[], return_id=False):
        ...
        if return_id:
            ...
            return self.pool['nh.clinical.patient'].browse(cr, uid, patient_ids)
...
with
    location_hierarchy  as (
        with recursive location_path(level,path,parent_id,id) as (
            select 0, id::text, parent_id, id from nh_clinical_location where parent_id is null
                union
            select level + 1, path||','||l.id, l.parent_id, l.id from nh_clinical_location l join location_path on l.parent_id = location_path.id
        )
    ...
    activity_parent_child_hierarchy  as (
        with recursive activity_path(level,path,parent_id,id) as (
            select 0, id::text, parent_id, id from nh_activity where parent_id is null
                union
            select level + 1, path||','||a.id, a.parent_id, a.id from nh_activity a join activity_path on a.parent_id = activity_path.id
        )    
        select id, ('{'||path||'}')::int[] as parent_ids 
        from activity_path
        order by path
    ...
    move_activity as (
        ...
        where data_model = 'nh.clinical.patient.move' and state = 'completed'
    ),
    spell_activity as (
        select 
            patient_id,
        ...
        from nh_activity
        where data_model = 'nh.clinical.spell'
    ...
    def get_adt_users(self, cr, uid, pos_ids=[], return_id=False):
        user_ids = self.user_map(cr, uid, pos_ids=pos_ids, group_xmlids=['group_nhc_adt']).keys()
        if return_id:
            return user_ids
        else:
    ...
    def get_locations(self, cr, uid, 
        ...
        if return_id:
            ...
            return self.pool['nh.clinical.location'].browse(cr, uid, location_ids)

    def get_occupied_bed_ids(self, cr, uid, parent_location_id):
        bed_ids = self.search(cr, uid, 'nh.clinical.location', [['id','child_of',parent_location_id],['usage','=','bed']])
    ...
    def activity_map(self, cr, uid, activity_ids=[], creator_ids=[],
        ...
        Returns:
            ...
            Format: {id: {id, ..., other params}}
            Parameters:
                id, all real fields of nh.activity: nh.clinical.patient field
                ...
                TODO:
                    ...
    ...
    def get_activities(self, cr, uid, activity_ids=[],
        ...
        if return_id:
            ...
            return self.pool['nh.activity'].browse(cr, uid, activity_ids)
    
    def get_activity_data(self, cr, uid, activity_id):
        activity = self.pool['nh.activity'].browse(cr, uid, activity_id)
    ...
    def location_map(self, cr, uid, location_ids=[], types=[], usages=[], codes=[], pos_ids=[],
        ...
        Returns:
            ...
            Parameters:
                id, code, type, usage, occupied, capacity: nh.clinical.location fields
            ...
            with
                move_patient_date as (
                    select 
                        ...
                        m.patient_id
                    from nh_clinical_patient_move m
                    inner join nh_activity a on m.activity_id = a.id
                ...
                patient_per_location as (
                    ...
                    from nh_clinical_patient_move m
                    inner join nh_activity ma on m.activity_id = ma.id
                    inner join move_patient_date mpd on m.patient_id = mpd.patient_id
                                                        and ma.sequence = mpd.max_sequence
                    inner join nh_activity sa on sa.data_model='nh.clinical.spell' and m.patient_id = sa.patient_id
                    --left join nh_activity a on a.location_id 
