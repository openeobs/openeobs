...
#                                                   creator_ids=[admission_activity_id]).keys()[0] 
    ...
    def build_uat_env(self, cr, uid, pos=1, ward='A', wm='winifred', nurse='norah', patients=8, placements=4, ews=1,
        ...
        # CHECK PARAMETERS
        assert self.pool['nh.clinical.pos'].read(cr, uid, pos, context=context)
        ...
        assert nurse_exists
        assert self.pool['nh.clinical.location'].search(cr, uid, [('code', '=', ward), ('usage', '=', 'ward'), ('user_ids', 'in', wm_exists)], context=context)
        assert patients > 0
        assert placements > 0
...
class nh_clinical_api_demo_data(orm.AbstractModel):
    ...
    _default_values_methods = {
        ...
        'nh.clinical.patient.observation.blood_sugar': 'observation_blood_sugar',
...
############# base ##############     
    ...
    def user_adt(self, cr, uid, values={}):
        ...
        if 'pos_id' not in values:
            ...
            v.update({'pos_id': api_demo.create(cr, uid, 'nh.clinical.pos')})
    ...
    def pos(self, cr, uid, values={}):
        ...
        if 'location_id' not in values:
            v.update({'location_id': api_demo.create(cr, uid, 'nh.clinical.location', 'location_pos')})
        if 'lot_admission_id' not in values:
            v.update({'lot_admission_id': api_demo.create(cr, uid, 'nh.clinical.location', 'location_admission')})
    ...
    def device(self, cr, uid, values={}):
        ...
        if not 'type_id' in values:
            type_id = fake.random_element(self.pool['nh.clinical.device.type'].search(cr, uid, []))
            if not type_id:
                api_demo = self.pool['nh.clinical.api.demo']
