...
class TestUsers(SingleTransactionCase):
    ...
    def setUp(self):
        ...
        self.responsibility_allocation = self.registry('t4.clinical.responsibility.allocation')
        self.userboard = self.registry('t4.clinical.userboard')
        self.userboard_admin = self.registry('t4.clinical.admin.userboard')
        self.apidemo = self.registry('t4.clinical.api.demo')
    ...
    def test_responsibility_allocation(self):
        ...
        spell_ids = self.activity_pool.search(cr, uid, [('data_model', '=', 't4.clinical.spell'), ('location_id', 'in', [ward_U_id, ward_T_id])])
        ...
        self.responsibility_allocation.submit(cr, uid, [ra_id])
        spell_ids = self.activity_pool.search(cr, uid, [('data_model', '=', 't4.clinical.spell'), ('location_id', 'in', [ward_T_id])])
        for spell_id in spell_ids:
