...
openerp.t4clinical_ui = function (instance) {
    ...
    /*instance.web.form.widgets.add('button', 'instance.t4clinical_ui.WidgetButton');
...
//    });
    ...
    instance.web.ListView.include({
        init: function(parent, dataset, view_id, options) {
            ...
            if (options.action){
                if (options.action.name == "Patients Board" || options.action.name == "Overdue Tasks" || options.action.name == "Doctor Tasks" || options.action.name == "Device Types" || options.action.name == "Devices" || options.action.name == "O2 Targets" || options.action.name == "User Management" || options.action.name == "Recently Discharged Patients" || options.action.name == "Patients without bed" || options.action.name == "Wardboard" || options.action.name == "Active Points of Care" || options.action.name == "Inactive Points of Care"){
        ...
        select_record: function (index, view) {
            ...
            if (this.fields_view.name != "T4 Clinical Placement Tree View"){
    ...
    instance.t4clinical_ui.WidgetButton = instance.web.form.WidgetButton.extend({
        on_click: function() {
            ...
            if ((this.field_manager.ViewManager.dataset.model == "t4.clinical.wardboard") && this.string == "Print Report"){
                // Do some shit
                // Old stuff below...
                this.model = new instance.web.Model("t4.clinical.wardboard");
                var attachment_id = this.model.call('print_report',[[this.view.datarecord.id]], {context: this.view.dataset.context}).done(function(response){
