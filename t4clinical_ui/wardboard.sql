...
completed_move as(
        select 
            data.patient_id,
            array_agg(data.id) as ids   
        from t4_clinical_patient_move data
        inner join t4_activity activity on activity.id = data.activity_id
        where activity.state = 'completed'
        group by data.patient_id
        )
...
--            array_agg(data.id) as ids   
--        from t4_clinical_spell spell
--        inner join t4_clinical_patient_o2target data on data.patient_id = spell.patient_id        
--        inner join t4_activity activity on activity.id = data.activity_id
--        where activity.state = 'completed'
--        group by spell.id
--        )
select 
    ...
    completed_spell.ids as wb_completed_spell_ids,
    completed_move.ids as wb_completed_move_ids
from t4_clinical_spell spell
...
left join started_device_session on started_device_session.patient_id = spell.patient_id
left join completed_device_session on completed_device_session.patient_id = spell.patient_id
left join completed_spell on completed_spell.patient_id = spell.patient_id
left join completed_move on completed_move.patient_id = spell.patient_id
