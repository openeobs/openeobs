...
class TestPBP(common.SingleTransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.pbp_monitoring_pool = cls.registry(
            'nh.clinical.patient.pbp_monitoring')
    ...
    def test_postural_blood_pressure_monitoring(self):
        ...
        pbp_activity_ids = self.activity_pool.search(
            cr, uid,
            [['data_model', '=', 'nh.clinical.patient.observation.pbp'],
             ['patient_id', '=', patient_id]])
        self.assertFalse(
            pbp_activity_ids,
            msg="Postural Blood Pressure monitoring activities were triggered "
                "before setting monitoring ON")
        # Set PBP Monitoring ON
        monitoring_id = self.pbp_monitoring_pool.create_activity(
            cr, uid, {'parent_id': spell_activity.id},
            {'patient_id': patient_id})
        ...
        self.assertTrue(
            check_monitoring.data_ref.patient_id.id == patient_id,
            msg="PBP Monitoring parameter set: Patient id not submitted "
                "correctly")
        self.assertTrue(
            check_monitoring.data_ref.status,
            msg="PBP Monitoring parameter set: Monitoring value not submitted "
                "correctly")
        self.activity_pool.complete(cr, uid, monitoring_id)
        check_monitoring = self.activity_pool.browse(cr, uid, monitoring_id)
        self.assertTrue(
            check_monitoring.state == 'completed',
            msg="PBP Monitoring parameter set not completed successfully")
        self.assertTrue(
            check_monitoring.date_terminated,
            msg="PBP Monitoring parameter set Completed: Date terminated not "
                "registered")
        ...
        self.assertTrue(
            check_pbp.data_ref.systolic_sitting ==
            pbp_data['systolic_sitting'],
            msg="PBP: Systolic sitting not submitted correctly")
        self.assertTrue(
            check_pbp.data_ref.systolic_standing ==
            pbp_data['systolic_standing'],
            msg="PBP: Systolic standing not submitted correctly")
        self.assertTrue(
            check_pbp.data_ref.diastolic_sitting ==
            pbp_data['diastolic_sitting'],
            msg="PBP: Diastolic sitting not submitted correctly")
        self.assertTrue(check_pbp.data_ref.diastolic_standing ==
                        pbp_data['diastolic_standing'],
                        msg="PBP: Diastolic standing not submitted correctly")
        # Complete PBP
        ...
        self.assertTrue(
            pbp_activity_ids,
            msg="PBP activity not scheduled after completing previous "
                "observation")
        ...
        self.assertFalse(
            notification_ids,
            msg="Inform FY2 triggered despite sitting and standing Blood "
                "Pressures being within the OK range")

        # Set PBP Monitoring OFF
        monitoring_id = self.pbp_monitoring_pool.create_activity(
            cr, uid,
            {'parent_id': spell_activity.id}, {'patient_id': patient_id})
        ...
        self.assertTrue(
            pbp_activity_ids,
            msg="PBP activity cancelled after setting monitoring OFF")
        ...
        self.assertTrue(
            ...
            msg="PBP: Diastolic sitting not submitted correctly")
        self.assertTrue(
            check_pbp.data_ref.diastolic_standing ==
            pbp_data['diastolic_standing'],
            msg="PBP: Diastolic standing not submitted correctly")
        # Complete PBP
        self.activity_pool.complete(cr, user_id, pbp_activity_ids[0])
        ...
        self.assertFalse(
            pbp_activity_ids,
            msg="PBP activity scheduled after completing previous observation"
                " despite monitoring being OFF")
        notification_ids = self.activity_pool.search(
            cr, uid,
            [['data_model', '=', 'nh.clinical.notification.nurse'],
             ['state', 'not in', ['completed', 'cancelled']],
        ...
        self.assertTrue(
            notification_ids,
            msg="Inform FY2 not triggered despite sitting and standing Blood "
                "Pressures having a difference > 20")
