...
class TestGetO2Display(SavepointCase):
    ...
    def setUp(self):
        ...
        del self.obs_data['oxygen_administration_flag']

    def call_test(self, oxygen_administration_flag=None, flow_rate=None,
                  expected=None):
        self.obs_data['patient_id'] = self.patient.id
        if oxygen_administration_flag is not None:
            self.obs_data['oxygen_administration_flag'] = \
                oxygen_administration_flag
        if flow_rate is not None:
            self.obs_data['flow_rate'] = flow_rate
        ews = self.ews_model.create(self.obs_data)

        actual = ews.o2_display
        self.assertEqual(expected, actual)

    def test_oxygen_administration_flag_is_not_given(self):
        self.call_test(oxygen_administration_flag=None, expected='')
    ...
    def test_oxygen_administration_flag_is_true_with_other_data(self):
        """
        Value should be an empty string when there is no data in the
        `flow_rate`, `concentration`, or `device_id` fields.
        """
        self.call_test(oxygen_administration_flag=True, flow_rate=10,
                       expected='10.0 l/m')
    ...
    def test_oxygen_administration_flag_is_false(self):
        ...
        """
        self.call_test(oxygen_administration_flag=False, expected='No')
