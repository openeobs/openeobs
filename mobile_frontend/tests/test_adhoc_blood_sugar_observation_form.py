...
class AdhocBSObsTest(common.SingleTransactionCase):
    ...
    def setUp(self):
        super(AdhocBSObsTest, self).setUp()
    ...
    def test_news_obs_form(self):
        ...
        if test_patient:
            ...
            patient = False
        form = dict()
        form['action'] = helpers.URLS['patient_form_action']+'{0}/{1}'.format('blood-sugar', test_patient['id'])
        ...
        form_desc = patient_api.get_form_description(cr, uid, test_patient['id'], 't4.clinical.patient.observation.blood_sugar', context=self.context)
