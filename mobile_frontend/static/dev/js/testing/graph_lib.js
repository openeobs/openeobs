graph_lib = (function() {
    ...
    var graph_lib = {
        ...
        drawChart: function() {
            ...
            context.area = d3.svg.line().interpolate("step-after").x(function(d) {
            ...
            context.obj.selectAll("circle").data(svg.data).enter().append("circle").attr("cx", function(d) {
                return context.xScale(d.date_start);
            }).attr("cy", function(d) {
                return context.yScale(d.score);
            ...
            svg.popup = d3.select(svg.el).append("div").attr("class", "contextPopup").style({"opacity": 1e-6,
                "position": "absolute",
                "text-align": "center",
                "width": "5%",
                "height": "inherit",
        ...
        drawGraph: function() {
            ...
            for (var i = 0; i < focus.graphs.length; i++) {
                ...
                if (thisEntry.label == "BP") {
                    focus.obj.selectAll("#" + thisEntry.label).data(svg.data).enter().append("rect").attr({
                        width: 2,
                        height: function(d) {
                    ...
                    thisEntry.area = d3.svg.line().interpolate("step-after").x(function(d) {
                        return focus.xScale(d.date_start);
                    }).y(function(d) {
                        return thisEntry.yScale(d[thisEntry.key]);
                    });
                    focus.obj.append("path").datum(svg.data).attr("d", thisEntry.area).style({
                        stroke: svg.printing ? "#222222" : "#888888",
                        "stroke-width": "1",
                        fill: "none"
                    }).attr("class", "focusPath").attr("id", "path-" + thisEntry.key).attr("clip-path", "url(#clip)");

                    focus.obj.selectAll("#" + thisEntry.label).data(svg.data).enter().append("circle").attr("cx", function(d) {
                        return focus.xScale(d.date_start);
                    }).attr("cy", function(d) {
                        return thisEntry.yScale(d[thisEntry.key]);
        ...
        drawTable: function() {
            ...
            for (var i = 0; i < focus.tables.length; i++) {
                ...
                tableTR.selectAll("td." + thisEntry.label).data(tableData).enter().append("td").text(function(d) {
                    return d;
