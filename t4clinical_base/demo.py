...
class t4_clinical_demo_env(orm.Model):
    ...
    def fake_data(self, cr, uid, env_id, model, data={}):
        ...
        method_map = {
            ...
            't4.clinical.location.ward': 'data_location_ward',
            't4.clinical.location.pos': 'data_location_pos',
            't4.clinical.location.admission': 'data_location_admission',
            't4.clinical.location.discharge': 'data_location_discharge',
            't4.clinical.pos': 'data_pos',
        }
...
#             self.submit_complete(cr, adt_user_id, env_id, placement_activity.id) 
    ...
    def build_bed_locations(self, cr, uid, env_id):
        ...
        for i in range(env.bed_qty): 
            d = self.fake_data(cr, uid, env_id, 't4.clinical.location.bed') 
    ...
    def build_ward_locations(self, cr, uid, env_id):
        ...
        for i in range(env.ward_qty): 
            d = self.fake_data(cr, uid, env_id, 't4.clinical.location.ward') 
            location_ids.append(location_pool.create(cr, uid, d)) 
    ...
    def build_pos(self, cr, uid, env_id):
        ...
        # POS Admission Lot
        d = self.fake_data(cr, uid, env_id, 't4.clinical.location.admission', {'parent_id': pos_location_id})
        lot_admission_id = location_pool.create(cr, uid, d)
        _logger.info("Admission location created id=%s data: %s" % (lot_admission_id, d))
        # POS Discharge Lot
        d = self.fake_data(cr, uid, env_id, 't4.clinical.location.discharge', {'parent_id': pos_location_id})  
