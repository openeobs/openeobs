...
class nh_clinical_api_extension(orm.AbstractModel):
    ...
    def change_activity_frequency(self, cr, uid, patient_id, activity_type,
        ...
        :mod:`review
        frequency<notifications.nh_clinical_notification_frequency>`
        ...
        frequency_activity_id = change_freq_pool.create_activity(
            ...
            {'patient_id': patient_id, 'observation': activity_type,
             'frequency': frequency})
        return activity_pool.complete(cr, uid, frequency_activity_id,
                                      context=context)
    ...
    def trigger_notifications(self, cr, uid, values, context=None):
        ...
        ``values``::

            {
                'notifications': [{
                    ...
                    'groups': (list of str) user groups for which
                              the notification is created, //required
        ...
        for n in values['notifications']:
            ...
            if values.get('group') in n['groups']:
                ...
                deadline = (
                    dt.now()+td(minutes=n.get('minutes_due'))).strftime(DTF) \
                ...
                if n['model'] == 'frequency':
                    ...
                    for f in activity_pool.browse(cr, uid,
                                                  frequency_activity_ids,
                                                  context=context):
                        if f.data_ref.observation == values.get('model'):
                            activity_pool.cancel(cr, uid, f.id,
                                                 context=context)
                    d_values.update({'observation': values.get('model')})
                pool.create_activity(cr, SUPERUSER_ID, a_values, d_values,
                                     context=context)
    ...
    def cancel_open_activities(self, cr, uid, parent_id, model, context=None):
        ...
        :param parent_id: :class:`activity<activity.nh_activity>` id of
        `nh.clinical.spell` type.
        :type parent_id: int
        ...
        domain = [('parent_id', '=', parent_id),
                  ('data_model', '=', model),
                  ('state', 'not in', ['completed', 'cancelled'])]
        open_activity_ids = activity_pool.search(cr, uid, domain,
                                                 context=context)
        return all([activity_pool.cancel(cr, uid, a, context=context)
