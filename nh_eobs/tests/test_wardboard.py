...
class TestWardboard(SingleTransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.devses_complete_pool = cls.registry(
            'wardboard.device.session.complete')
        ...
        cls.wb_disc_id = cls.wardboard_pool.search(
            cr, uid, [['spell_state', '=', 'started'],
        ...
        cls.wb_id = cls.wardboard_pool.search(
            cr, uid, [['spell_state', '=', 'started'],
                      ['patient_id', '=', cls.patients[0]]])[0]
        cls.wb_id2 = cls.wardboard_pool.search(
            cr, uid, [['spell_state', '=', 'started'],
                      ['patient_id', '=', cls.patients[1]]])[0]
        cls.wb_id3 = cls.wardboard_pool.search(
            cr, uid, [['spell_state', '=', 'started'],
    ...
    def test_01_swap_beds(self):
        ...
        swap_id = self.swap_pool.create(
            cr, self.wm_uid, {'patient1_id': self.patients[0],
                              'ward_location_id': self.ward_id,
                              'location1_id': self.beds[0]})
        # Scenario 1: onchange_location2 empty location
        self.assertDictEqual(self.swap_pool.onchange_location2(
            cr, self.wm_uid, swap_id, False),
            {'value': {'patient2_id': False}})

        # Scenario 2: onchange_location2 location without patient
        self.assertDictEqual(self.swap_pool.onchange_location2(
            cr, self.wm_uid, swap_id, self.beds[2]),
            {'value': {'patient2_id': False, 'location2_id': False}})

        # Scenario 3: onchange_location2 location with patient
        self.assertDictEqual(self.swap_pool.onchange_location2(
            cr, self.wm_uid, swap_id, self.beds[1]),
            {'value': {'patient2_id': self.patients[1]}})

        # Scenario 4: do_swap creates a swap activity
        self.swap_pool.write(
            cr, self.wm_uid, swap_id,
            {'patient2_id': self.patients[1], 'location2_id': self.beds[1]})
        self.swap_pool.do_swap(cr, self.wm_uid, [swap_id])
        self.assertTrue(self.swapbeds_pool.search(
            cr, uid, [['location1_id', '=', self.beds[0]],
    ...
    def test_02_swap_beds_open_wizard(self):
        ...
        res = self.wardboard_pool.wardboard_swap_beds(
    ...
    def test_04_placement_open_wizard(self):
        ...
        res = self.wardboard_pool.wardboard_patient_placement(
            cr, self.wm_uid, [self.wb_id])
        ...
        with self.assertRaises(except_orm):
            self.wardboard_pool.wardboard_patient_placement(
                cr, self.wm_uid, [self.wb_id3])

    def test_05_device_session_start(self):
        cr, uid = self.cr, self.uid

        devtype_id = self.devtype_pool.search(cr, uid, [])[0]
        devcategory_id = self.devtype_pool.browse(
            cr, uid, devtype_id).category_id.id
        device_id = self.device_pool.create(
            cr, uid, {'type_id': devtype_id, 'serial_number': '000111'})

        devses_start_id = self.devses_start_pool.create(cr, uid, {
            'patient_id': self.patients[0],
            'device_category_id': devcategory_id, 'device_type_id': devtype_id,
            'device_id': device_id, 'location': 'mouth'})

        # Scenario 1: onchange_device_category_id no category
        self.assertFalse(self.devses_start_pool.onchange_device_category_id(
            cr, uid, [devses_start_id], False))

        # Scenario 2: onchange_device_category_id
        ids = self.devtype_pool.search(
            cr, uid, [('category_id', '=', devcategory_id)])
        self.assertDictEqual(
            self.devses_start_pool.onchange_device_category_id(
                cr, uid, [devses_start_id], devcategory_id),
            {'value': {'device_id': False, 'device_type_id': False},
             'domain': {'device_type_id': [('id', 'in', ids)]}})

        # Scenario 3: onchange_device_category_id no type
        self.assertFalse(self.devses_start_pool.onchange_device_type_id(
            cr, uid, [devses_start_id], False))

        # Scenario 4: onchange_device_type_id
        ids = self.device_pool.search(cr, uid, [('type_id', '=', devtype_id)])
        self.assertDictEqual(
            self.devses_start_pool.onchange_device_type_id(
                cr, uid, [devses_start_id], devtype_id),
            {'value': {'device_id': False},
             'domain': {'device_id': [('id', 'in', ids),
                                      ('is_available', '=', True)]}})

        # Scenario 5: onchange_device_id no device
        self.assertFalse(self.devses_start_pool.onchange_device_id(
            cr, uid, [devses_start_id], False))

        # Scenario 6: onchange_device_type_id
        self.assertDictEqual(
            self.devses_start_pool.onchange_device_id(
                cr, uid, [devses_start_id], device_id),
    ...
    def test_06_device_session_start_open_wizard(self):
        ...
        res = self.wardboard_pool.device_session_start(
            cr, self.wm_uid, [self.wb_id])
        res_id = self.devses_start_pool.search(
            cr, uid, [], order="id desc")[0]
        view_id = self.model_data.get_object_reference(
            cr, uid, 'nh_eobs',
    ...
    def test_07_device_session_complete(self):
        ...
        devses_complete_id = self.devses_complete_pool.create(
            cr, uid,
            {'session_id': activity.data_ref.id,
             'removal_reason': 'No reason', 'planned': 'planned'})
        res = self.devses_complete_pool.do_complete(
            cr, self.wm_uid, [devses_complete_id])
        activity = self.activity_pool.browse(cr, uid, activity_id[0])
        view_id = self.model_data.get_object_reference(
    ...
    def test_08_device_session_complete_open_wizard(self):
        ...
        activity_id = self.activity_pool.search(cr, uid, [
            ...
            ['state', '=', 'completed']])
        activity = self.activity_pool.browse(cr, uid, activity_id[0])
        res = self.dev_session_pool.device_session_complete(
            cr, uid, [activity.data_ref.id])
        res_id = self.devses_complete_pool.search(
            cr, uid, [], order="id desc")[0]
        view_id = self.model_data.get_object_reference(
            cr, uid, 'nh_eobs',
        ...
        self.assertDictEqual(res, {
            'name': "Complete Device Session: %s"
                    % activity.data_ref.patient_id.full_name,
            'type': 'ir.actions.act_window',
    ...
    def test_09_get_logo(self):
        ...
        res = self.wardboard_pool._get_logo(
            cr, uid, [self.wb_id], 'company_logo', None)
        self.assertEqual(res[self.wb_id], patient.partner_id.company_id.logo)

    def test_10_fields_view_get(self):
        cr, uid = self.cr, self.uid

        res = self.wardboard_pool.fields_view_get(
            cr, self.nurse_uid, view_id=False, view_type='form')
        self.assertTrue(res['fields']['o2target']['readonly'])
        res = self.wardboard_pool.fields_view_get(
            cr, self.wm_uid, view_id=False, view_type='form')
        self.assertFalse(res['fields']['o2target']['readonly'])
        res = self.wardboard_pool.fields_view_get(
            cr, self.wm_uid, view_id=False, view_type='tree')
        self.assertTrue(res)
        view_id = self.model_data.get_object_reference(
            cr, uid, 'nh_eobs', 'view_wardboard_chart_form')[1]
        res = self.wardboard_pool.fields_view_get(
            cr, self.wm_uid, view_id=view_id, view_type='form')
        self.assertTrue(res)
    ...
    def test_11_get_started_device_session_ids(self):
        ...
        devcategory_id = self.devtype_pool.browse(
            cr, uid, devtype_id).category_id.id
        device_id = self.device_pool.create(
            cr, uid, {'type_id': devtype_id, 'serial_number': '000112'})
        ...
        res = self.wardboard_pool._get_started_device_session_ids(
            cr, self.wm_uid, [self.wb_id], 'started_device_session_ids', None)
    ...
    def test_12_get_terminated_device_session_ids(self):
        ...
        res = self.wardboard_pool._get_terminated_device_session_ids(
            cr, self.wm_uid, [self.wb_id],
            'terminated_device_session_ids', None)
        ids = self.dev_session_pool.search(cr, uid, [
            ['patient_id', '=', self.patients[0]],
            ['activity_id.state', 'in', ['completed', 'cancelled']]])
        self.assertListEqual(res[self.wb_id], ids)
    ...
    def test_15_get_transferred_user_ids_search(self):
        ...
        res = self.wardboard_pool._transferred_user_ids_search(
            cr, uid, 'nh.clinical.wardboard', 'transferred_user_ids',
            [['transferred_user_ids', 'in', [self.wm_uid]]])
        self.assertListEqual(res, [('id', 'in', [self.wb_id2])])

    def test_16_prescribe_action(self):
        cr, uid = self.cr, self.uid

        res = self.wardboard_pool.wardboard_prescribe(
            cr, self.wm_uid, [self.wb_id])
        wardboard = self.wardboard_pool.browse(cr, uid, self.wb_id)
        view_id = self.model_data.get_object_reference(
            cr, uid, 'nh_eobs', 'view_wardboard_prescribe_form')[1]
        self.assertDictEqual(res, {
    ...
    def test_17_news_chart_action(self):
        ...
        res = self.wardboard_pool.wardboard_chart(
            cr, self.wm_uid, [self.wb_id])
        wardboard = self.wardboard_pool.browse(cr, uid, self.wb_id)
        view_id = self.model_data.get_object_reference(
            cr, uid, 'nh_eobs', 'view_wardboard_chart_form')[1]
    ...
    def test_21_placement_action(self):
        ...
        res = self.wardboard_pool.wardboard_place(
            cr, self.wm_uid, [self.wb_id3], {})
        ...
        view_id = self.model_data.get_object_reference(
            cr, uid, 'nh_clinical', 'view_patient_placement_complete')[1]
        self.assertDictEqual(res, {
    ...
    def test_22_write(self):
        ...
        self.assertTrue(self.wardboard_pool.write(
            cr, self.wm_uid, [self.wb_id], {'mrsa': 'no'}))
        ...
        self.assertTrue(self.wardboard_pool.write(
            cr, self.wm_uid, [self.wb_id], {'diabetes': 'no'}))
        activity_ids = self.activity_pool.search(cr, uid, [
            ['patient_id', '=', self.patients[0]], ['state', '=', 'completed'],
        ...
        self.assertTrue(self.wardboard_pool.write(
            cr, self.wm_uid, [self.wb_id], {'pbp_monitoring': 'no'}))
        activity_ids = self.activity_pool.search(cr, uid, [
            ['patient_id', '=', self.patients[0]], ['state', '=', 'completed'],
        ...
        self.assertTrue(self.wardboard_pool.write(
            cr, self.wm_uid, [self.wb_id], {'o2target': False}))
        activity_ids = self.activity_pool.search(cr, uid, [
            ['patient_id', '=', self.patients[0]], ['state', '=', 'completed'],
        ...
        self.assertTrue(self.wardboard_pool.write(
            cr, self.wm_uid, [self.wb_id], {'palliative_care': 'no'}))
        activity_ids = self.activity_pool.search(cr, uid, [
            ['patient_id', '=', self.patients[0]], ['state', '=', 'completed'],
    ...
    def test_23_get_cr_groups(self):
        ...
        res, fold = self.wardboard_pool._get_cr_groups(
            cr, self.wm_uid, [self.wb_id], [])
        groups = [['NoScore', 'No Score Yet'], ['High', 'High Risk'],
    ...
    def test_24_open_previous_spell_action(self):
        ...
        res = self.wardboard_pool.open_previous_spell(
            cr, self.wm_uid, [self.wb_id2])
        ...
        activity_ids = self.activity_pool.search(cr, uid, [
            ...
            ['sequence', '<', wardboard.spell_activity_id.sequence],
            ['state', '=', 'completed']
        ], order='sequence desc')
        res_id = self.activity_pool.browse(
            cr, uid, activity_ids[0]).data_ref.id
        self.assertDictEqual(res, {
            'name': 'Previous Spell',
            'type': 'ir.actions.act_window',
            'res_model': 'nh.clinical.wardboard',
        ...
        with self.assertRaises(except_orm):
            self.wardboard_pool.open_previous_spell(
                cr, self.wm_uid, [self.wb_id])
    ...
    def test_25_get_recently_discharged_uids(self):
        ...
        self.assertListEqual(sorted(res[self.wb_disc_id]), sorted(
            [self.wm_uid, self.dr_uid]))
