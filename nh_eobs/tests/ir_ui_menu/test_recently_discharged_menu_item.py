...
class TestRecentlyDischargedMenuItem(MenuItemCase):
    """
    Test that the Recently Discharged menu item is shown or hidden for various
    user groups
    """
    ...
    def setUp(self):
        ...
        Set up the tests
        """
        super(TestRecentlyDischargedMenuItem, self).setUp()
        self.allowed_groups = \
            self.get_groups_for_menu_item('Recently Discharged')
