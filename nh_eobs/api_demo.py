...
class nh_clinical_api_demo(orm.AbstractModel):
    ...
    def demo_loader(self, cr, uid, config_file, return_file):
        ...
        locations = self.generate_locations(
            cr, uid, wards=config['wards'], beds=config['beds'], hospital=True)
        ...
        patient_ids = self._load_patients(
            cr, adt_uid, config['wards'], config['patients'])
        admitted_patient_ids = self._load_admit_patients(
            cr, adt_uid, ward_ids, patient_ids, config['days'])
        ...
        if return_file:
            ...
            with open(
                    os.path.join(directory_name, return_file), 'w'
            ) as outfile:
    ...
    def _load_users(self, cr, uid, locations, users):
        ...
        for k in locations:
            result.append(
                self.generate_users(cr, uid, locations[k], data=users))
        return result
    ...
    def _load_admit_patients(self, cr, uid, ward_ids, patient_ids, days):
        ...
        for n in range(len(patient_ids)):
            data = {
                'location': ward_codes[n]['code'],
                'start_date': start_date}
            admitted_patient_ids.append(
                self.admit_patients(cr, uid, patient_ids[n], data))
    ...
    def _load_place_patients(self, cr, uid, ward_ids, patient_ids,
        ...
        for n in range(len(ward_ids)):
            bed_ids.append(
                self.place_patients(
                    cr, uid, patient_ids[n], ward_ids[n], context=context))
        return bed_ids
    ...
    def generate_locations(self, cr, uid, wards=0, beds=0, hospital=False):
        ...
        if hospital:
            hospital_id = location_pool.create(
                cr, uid, {'name': fake.company()})
            admission_id = location_pool.create(
                cr, uid, {'name': fake.company()})
            discharge_id = location_pool.create(
                cr, uid, {'name': fake.company()})
            company_id = company_pool.create(
                cr, uid, {'name': fake.company()})
    ...
    def generate_users(self, cr, uid, location_ids, data=None):
        ...
        pos_id = location_pool.read(
            cr, uid, [location_ids[0]], ['pos_id'])[0]['pos_id'][0]

        # create adt user if non exists
        adt_group_id = group_pool.search(
            cr, uid,
            [['name', 'in',
              ['NH Clinical ADT Group', 'Contact Creation',
               'NH Clinical Admin Group', 'Employee']]])
        adt_uid_ids = user_pool.search(
            cr, uid,
            [['groups_id', 'in', adt_group_id], ['pos_id', '=', pos_id]])
        if adt_uid_ids:
    ...
    def admit_patients(self, cr, uid, patient_ids, data, context=None):
        ...
        for patient_id in patient_ids:
            ...
            if not spell_activity_id:
                if api.admit(
                        cr, uid, patient.other_identifier,
                        data, context=context):
                    identifiers.append(patient_id)
                    _logger.info(
                        "Patient '%s' admitted", patient.other_identifier)
        return identifiers
        ...
        for index, bed_id in enumerate(bed_ids):
            if index < len(activity_ids):
                activity_pool.submit(
                    cr, uid, activity_ids[index], {'location_id': bed_id})
    ...
    def generate_news_simulation(self, cr, uid, begin_date=False,
        ...
        if uid == SUPERUSER_ID:
            ...
            nurse_group_id = group_pool.search(
                cr, uid,
                [['name', '=', 'NH Clinical Nurse Group']], context=context)
            user_pool.write(
                cr, uid, SUPERUSER_ID,
        ...
        while current_date < dt.now():
            ...
            for ews_id in ews_activity_ids:
                ...
                activity_pool.submit(
                    cr, uid, ews_id, ews_data, context=context)
    ...
    def build_unit_test_env(self, cr, uid, wards=None, bed_count=2,
                            patient_admit_count=2, patient_placement_count=1,
                            ews_count=1, context=False, weight_count=0,
                            blood_sugar_count=0, height_count=0,
                            o2target_count=0, users=None):
        """
        ...
        ward_ids = [self.create(
            ...
            {'context_ids': context, 'parent_id': pos_location_id,
             'name': 'Ward '+w, 'code': w}) for w in wards]
        i = 0
        bed_ids = {}
        bed_codes = {}
        for wid in ward_ids:
            bed_ids[wards[i]] = [self.create(
                ...
                {'context_ids': context, 'parent_id': wid,
                 'name': 'Bed '+str(n), 'code': wards[i]+str(n)}
        ...
        for wid in ward_ids:
            ...
            for n in range(patient_placement_count):
                ...
                activity_pool.submit(
                    cr, wmuid, placement_activity_id,
            ...
            for wid in ward_ids:
                ews_activity_ids += activity_pool.search(
                    ...
                    [('data_model', '=',
                     ...
                     ('state', 'not in', ['completed', 'cancelled']),
                     ('location_id', 'child_of', wid)])
            for ews in activity_pool.browse(cr, uid, ews_activity_ids):
                nuid = user_pool.search(
                    cr, uid,
                    [('location_ids', 'in', [ews.location_id.id]),
