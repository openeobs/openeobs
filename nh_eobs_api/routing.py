...
class Route(object):
    ...
    def _split_url(self, url, split_on='/'):
        ...
        Also, remove empty pieces, usually generated by two consecutive occurrences of the separator in the URL.
        This is supposed to be an utility method, only used internally by other higher-level methods.

        :param url: String (not necessarily a well-formed URL)
        :param split_on: String used as separator to determine where to split the URL
        :return: List of comma separated strings, one for each piece the URL is split into
        :raise ValueError: If the 'url' argument passed is not a string - hence, it does not have a split() method
        """
    ...
    def _get_url_components(self, url):
        ...
        :return: List of dictionaries, each containing one URL component, alongside metadata describing it
        :raise ValueError: If, when parsing a piece of the URL passed as argument, that piece results not to be a string
        """
        decomposed_url = []
        url_pieces_list = self._split_url(url)
        for piece in url_pieces_list:
            if not isinstance(piece, basestring):
                raise ValueError('Parsed {} as piece of URL, but it is not a string as expected.'.format(piece))
            decomposed_url.append(
                {
                    'type': 'func' if piece.startswith('<') and piece.endswith('>') else 'string',
                    'name': piece.lstrip('<').rstrip('>')  # store just the actual name of the URL piece
                }
            )
