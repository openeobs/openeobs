...
from nh_eobs_mobile.controllers.urls import routes
from nh_eobs_mobile.controllers.urls import URLS
# edited above 2 imports as it couldn't find them
...
class TestMobileControllerMethods(tests.common.HttpCase):
    ...
    def setUp(self):
        ...
        if not self.auth_resp.status_code == 200:
            self._safely_exit_test_during_setup(
                exit_type='skip',
                reason='Cannot retrieve an authenticated session '
                       'to be used for the test.'
            )
        if 'class="tasklist"' not in self.auth_resp.content:
            self._safely_exit_test_during_setup(
                exit_type='skip',
                reason="Cannot reach the 'task list' page after "
                       "the authentication (during test set up)."
            )
...
class TestGetSingleTaskMethod(tests.common.HttpCase):
    ...
    def setUp(self):
        ...
        self.task_list_url = self._build_url(task_list_route[0]['endpoint'],
                                             None)
        if self.task_list_url not in self.auth_resp.url:
            self._safely_exit_test_during_setup(
                exit_type='skip',
                reason="Cannot reach the 'task list' page after "
                       "the authentication (during test set up)."
