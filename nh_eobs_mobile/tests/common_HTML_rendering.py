...
class MobileHTMLRenderingCase(common.TransactionCase):
    """Parent test case for all the test cases related to the HTML rendering by the mobile controller."""
    ...
    def setUpClass(cls):
        ...
        cls.fixtures_directory = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'fixtures')
    ...
    def _render_template(self, cr, uid, template_id=None, options=None):
        """Rely on Odoo's 'view' model to render a specific template with a specific context.
        :param template_id: A string with the template's ID (as stored in the Odoo's database)
        :param options: A dictionary which will be used to render the template (as Odoo's 'qcontext')
        """
        return self.registry('ir.ui.view').render(cr, uid, template_id, options, context=self.context)
