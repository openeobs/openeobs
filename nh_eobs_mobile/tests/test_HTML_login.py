...
class TestLoginHTML(MobileHTMLRenderingCase):
    """
    Test case collecting all the tests relating to the RENDERING
    of the 'login' page.
    ...
    def test_login_page_in_a_clean_state(self):
        ...
        i.e. the user has just arrived in the page and the form
        is still pristine.
        """
        ...
        # Set up specific data to provide them to the
        ...
        rendered_template = self.jinja_env.\
            get_template('login.html').render(**api_data)
        compressed_rendered_template = self._compress_string(
            rendered_template)
        compressed_expected_output = self._compress_string(
            expected_output)
        self.assertEqual(
    ...
    def test_login_page_in_an_invalid_state(self):
        ...
        i.e. the user has already submit wrong credentials,
        now he's been showing a login page with an error message.
        """
        # Retrieve the fixture for this test and parse it,
        # thus there is an expected output to compare the mobile
        # controller's method's result against
        ...
        # Set up specific data to provide them to the template
        # for the rendering
        err = '<div class="alert alert-error">Invalid username/password</div>'
        api_data = {
            'stylesheet': '/mobile/src/css/main.css',
            'logo': '/mobile/src/img/logo.png',
            'form_action': '/mobile/login/',
        ...
        rendered_template = self.jinja_env.get_template('login.html')\
            .render(**api_data)
        compressed_rendered_template = self._compress_string(
            rendered_template)
        compressed_expected_output = self._compress_string(
