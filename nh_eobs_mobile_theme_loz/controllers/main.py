...
loader = jinja2.FileSystemLoader(get_module_path('nh_eobs_mobile_theme_loz') + '/views/')
...
class LozStyleFrontend(openerp.addons.nh_eobs_mobile.controllers.main.MobileFrontend):

    @http.route(URLS['stylesheet'], type='http', auth='none')
    def get_stylesheet(self, *args, **kw):
        with open(get_module_path('nh_eobs_mobile_theme_loz') + '/static/src/css/style.css', 'r') as stylesheet:
    ...
    def mobile_logout(self, *args, **kw):
        database_url = URLS['login']+'?database={db}'.format(db=request.session.db) if request.session.db else URLS['login']
        request.session.logout()
        return utils.redirect(database_url, 303)

    @http.route('/mobile/src/img/<xmlid>', auth='none', type='http')
    def get_image(self, xmlid, *args, **kw):
        with open(get_module_path('nh_eobs_mobile_theme_loz') + '/static/src/img/' + xmlid, 'r') as image:
    ...
    def get_logo(self, *args, **kw):
        with open(get_module_path('nh_eobs_mobile_theme_loz') + '/static/src/img/logo.png', 'r') as logo:
    ...
    def convert_patient_ews_deadline(self, deadline):
        overdue = False
        deadline_to_return = deadline
        if 'overdue:' in deadline:
            overdue = True
            deadline_to_return = deadline.replace('overdue: ', '')
        return [deadline_to_return, overdue]
